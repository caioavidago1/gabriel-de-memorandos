"""
Template Loader para Short Memo - Primário (Gestora)
...
class SectionTemplate:
    ...
    tone: str
    facts_sections_used: Dict[str, Dict[str, List[str]]]
...
"""
        ...
        # Adicionar campos obrigatórios e recomendados
        mandatory = []
        recommended = []
        for section_name, fields in self.facts_sections_used.items():
            mandatory.extend(fields.get('mandatory', []))
            recommended.extend(fields.get('recommended', []))
    ...
    def validate_facts(self, facts: Dict[str, Any]) -> Dict[str, Any]:
        ...
        return {
            ...
            'coverage_pct': (
                (len(mandatory) - len(missing_must)) / 
                len(mandatory) * 100
...
class TemplateLoader:
    ...
    def _load_templates(self) -> Dict[str, SectionTemplate]:
        ...
        for section_key, section_data in data.get('templates', {}).items():
            templates[section_key] = SectionTemplate(
                ...
                facts_sections_used=section_data.get('facts_sections_used', {}),
    ...
    def enrich_prompt(
        ...
        Args:
            section: Nome da seção (ex: 'intro', 'gestora')

...
def _flatten_facts_sections_used(facts_sections_used: Dict[str, Any]) -> tuple:
    """
    Converte facts_sections_used (mandatory/recommended por seção) em listas planas
    must_include e recommended_include, para compatibilidade com SectionTemplate.
    ...
    seen_must = set()
    seen_rec = set()
    for _section, opts in (facts_sections_used or {}).items():
        if isinstance(opts, dict):
            for f in (opts.get('mandatory') or []):
                if f not in seen_must:
                    seen_must.add(f)
                    must.append(f)
            for f in (opts.get('recommended') or []):
                if f not in seen_rec:
                    seen_rec.add(f)
...
class TemplateLoader:
    ...
    def _load_templates(self) -> Dict[str, SectionTemplate]:
        ...
        for section_key, section_data in data.get('templates', {}).items():
            # Suportar facts_sections_used (mandatory/recommended por seção) quando must_include/recommended_include não existem
            must_include = section_data.get('must_include')
            recommended_include = section_data.get('recommended_include')
            if must_include is None or recommended_include is None:
                must_include, recommended_include = _flatten_facts_sections_used(
                    section_data.get('facts_sections_used', {})
                )
            if must_include is None:
                must_include = []
            if recommended_include is None:
                recommended_include = []

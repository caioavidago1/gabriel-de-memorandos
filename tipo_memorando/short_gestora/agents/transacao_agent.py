"""
Agente de Transação - Short Memo Gestora

Responsável por gerar a seção "Transação" com valuation e estrutura.
"""
...
from ..facts_utils import get_name_safe, get_numeric_safe, get_currency_safe
from ..._base.format_utils import get_currency_symbol, get_currency_label


class TransacaoAgent:
    """Agente especializado em geração de Transação para Short Memo Gestora."""
    
    def __init__(self, model: str = "gpt-4o", temperature: float = 0.25):
        self.model = model
        self.temperature = temperature
        self.section_name = "Transação"
    ...
    def generate(
        ...
        # ===== SYSTEM PROMPT =====
        system_prompt = f"""Você é um analista sênior de Private Equity com 15 anos de experiência escrevendo a seção "Transação" de um Short Memo de Co-investimento para o comitê de investimento da Spectra Capital.
...
§ PARÁGRAFO 2 - Análise da Estrutura (2-4 frases):
  ...
  - Governança: "A estrutura de governança prevê [conselho, vetos, tag/drag, vesting, lock-up]"

§ PARÁGRAFO 3 - Retornos Esperados (3-4 frases):
  - Cenário base: "Considerando o cenário base ([premissas principais]) e saída em {exit_year} a {exit_multiple}x EBITDA, os retornos projetados são de {irr_pct or 'X'}% TIR e {moic or 'Y,Z'}x MOIC."
  - Premissas críticas: "O modelo assume [premissa 1], [premissa 2] e [premissa 3]."
...
✅ "estrutura de governança", "conselho consultivo", "direitos de veto"
...
6. Mantenha tom profissional e crítico

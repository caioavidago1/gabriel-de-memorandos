"""
Agente de Pontos a Aprofundar - Short Memo Gestora
...
class PontosAprofundarAgent:
    """Agente especializado em geração de Pontos a Aprofundar para Short Memo Gestora."""
    ...
    def generate(
        ...
        key_risks = get_text_safe(facts, "qualitativo", "key_risks")
        ...
        # ===== SYSTEM PROMPT =====
        system_prompt = f"""Você é um analista sênior de Private Equity com 15 anos de experiência escrevendo a seção "Pontos a Aprofundar" de um Short Memo de Co-investimento para o comitê de investimento da Spectra Capital.
...
"Dito isso, acreditamos ser necessário aprofundar os seguintes pontos:"
...
✗ "É necessário aprofundar..." (não use conjugações, use infinitivo)
...
4. **Riscos Operacionais:**
   - Key man risk (dependência de fundadores/gestão)
   - Concentração de clientes/fornecedores
   - Complexidade operacional
   - Transição de management
   - Dependências críticas (parcerias, contratos)
...
6. **Governança e Alocação:**
   - Estrutura de governança pós-aquisição
   - Alocação do gestor líder e co-investidores
...
**OUTPUT:** Comece com a frase de abertura obrigatória, seguida pela lista de pontos (5-8 bullets), sem título de seção, sem categorias."""
...
1. Comece com a frase de abertura obrigatória: "Dito isso, acreditamos ser necessário aprofundar os seguintes pontos:"
2. Gere 5-8 pontos em formato de lista com hífen "-"
3. Cada ponto começa com verbo (Aprofundar/Validar/Entender/Definir/Montar)
4. Seja ESPECÍFICO - inclua números, nomes, valores quando relevante
5. Máximo 2 linhas por ponto
6. Priorize riscos mencionados explicitamente nos facts ou RAG
7. Se dados limitados, foque em validações padrão de DD
8. NÃO numere, NÃO use categorias, NÃO quebre linhas
9. Base-se EXCLUSIVAMENTE em informações disponíveis

Comece AGORA com a frase de abertura seguida pelos pontos:"""
